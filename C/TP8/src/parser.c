...
static void tokenize(Stack *st, char *token, int *quit_opt, Plugin_list *plugins){
   ...
   if(is_numeric(token)){
       ...
       if(!token){/*Token is null*/
           ...
           tokenize(st, token, quit_opt, plugins);
       }
   }
   else{
       switch(input_kind(token[0])){
            case OPERATOR:
                /*eval_operator(st, token[0]);*/
                eval_by_plugins(st, plugins, token[0]);
                break;
            case OPT:
        ...
        tokenize(st, post_token, quit_opt, plugins);
        free(post_token);
   }
}
...
int parse(Stack *st, char *input, int *quit_opt, Plugin_list *plugins){
    ...
    while(tokens != NULL){
        ...
        tokenize(st, tokens, quit_opt, plugins);
